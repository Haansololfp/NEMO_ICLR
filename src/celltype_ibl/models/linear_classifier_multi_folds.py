...
def process_fold(
    ...
    if embedding_model == "contrastive":
        ...
        checkpoint = torch.load(embedding_model_path, map_location=torch.device("cpu"))
...
def cross_validate(
    ...
    embedding_model: str = "contrastive",
    embedding_model_paths: list[str] | None = None,
    ...
    if embedding_model == "VAE":
        ...
        assert embedding_model_paths is not None
    ...
    assert embedding_model_paths is not None
    assert len(embedding_model_paths) == n_splits
    ...
    for run_id in range(n_runs):
        ...
        kfold = (
            CrossSessionLeaveOneOut(session_idx).split(dataset, targets)
            if by_session
            else (LeaveOneOut().split(dataset, targets) if loo else k_fold)
        )
        ...
        fold_args = [
            ...
            for fold_id, (train_idx, val_idx) in enumerate(kfold)
        ]
...
def main(
    ...
    results_dict = cross_validate(
        ...
        embedding_model=embedding_model,
        embedding_model_paths=embedding_model_paths,
