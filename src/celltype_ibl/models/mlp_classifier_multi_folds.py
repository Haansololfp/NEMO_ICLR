...
def cross_validate(
    ...
    for _ in tqdm(range(n_runs), desc="Cross-validation run", position=0, leave=True):
        ...
        kfold = LeaveOneOut().split(dataset, targets) if loo else k_fold
        ...
        for fold, (train_idx, val_idx) in tqdm(
            ...
            total=len(dataset) if loo else n_splits,
        ):
            ...
            if embedding_model == "VAE":
                ...
                embedding_model_path = embedding_model_paths[fold]
        ...
        run_f1 = f1_score(
            run_true_targets, run_model_pred, average="weighted"
        )  # , average="macro")
